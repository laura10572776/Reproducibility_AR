...
ABSL_FLAG(std::string, game, "go(board_size=5,use_score=true,komi=0.0)", "The name of the game to play.");
...
ABSL_FLAG(int, nn_width, 100, "Width of the model, passed to export_model.py.");
ABSL_FLAG(int, nn_depth, 3, "Depth of the model, passed to export_model.py.");
...
ABSL_FLAG(double, temperature_drop, 0, // Smaller than AZ due to short games.
          "Drop the temperature to 0 after this many moves.");
ABSL_FLAG(double, cutoff_probability, 0.0,
          ("Cut off rollouts early when above the cutoff value with this "
           "probability."));
ABSL_FLAG(double, cutoff_value, 25,
...
ABSL_FLAG(int, checkpoint_freq, 1, "Save a checkpoint every N steps.");
ABSL_FLAG(int, max_simulations, 10, "How many simulations to run.");
...
ABSL_FLAG(int, max_steps, 1, "How many learn steps to run.");
ABSL_FLAG(int, evaluation_window, 100, "Number of games to average results over.");
ABSL_FLAG(double, pw_exp, 1.0, "Progressive widening exponent.");
ABSL_FLAG(double, ar_factor, 10.0, "Abstraction refining factor.");
ABSL_FLAG(double, ar_exp, 1.0, "Abstraction refining exponent");
ABSL_FLAG(bool, use_ar, false, "Whether to use abstraction refining.");
ABSL_FLAG(bool, perturb, false, "Whether to disallow state identification.");
ABSL_FLAG(int, jobnum, 0, "Job number.");
ABSL_FLAG(int, num_games, 100, "Number of games to play.");
...
int main(int argc, char **argv)
{
  ...
  open_spiel::algorithms::torch_az_eval::AlphaZeroConfig config;
  ...
  config.pw_exp = absl::GetFlag(FLAGS_pw_exp);
  config.ar_factor = absl::GetFlag(FLAGS_ar_factor);
  config.ar_exp = absl::GetFlag(FLAGS_ar_exp);
  config.use_ar = absl::GetFlag(FLAGS_use_ar);
  config.perturb = absl::GetFlag(FLAGS_perturb);
  config.jobnum = absl::GetFlag(FLAGS_jobnum);
  config.num_games = absl::GetFlag(FLAGS_num_games);
