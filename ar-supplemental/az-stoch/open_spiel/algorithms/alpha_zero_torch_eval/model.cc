...
#include "open_spiel/algorithms/alpha_zero_torch_eval/model.h"
  ...
  namespace algorithms
  {
    namespace torch_az_eval
    {
                                                             ...
                                                             torch::Tensor mask)
      {
        ...
        value_output = value_linear2_(value_output);
                                                       ...
                                                       torch::Tensor mask)
      {
        ...
        // cheap temperature hack
        double temperature;
        if (output[2].sizes()[1] == 64)
        {
          temperature = 1.0;
        }
        else
        ...
        return {output[0], torch::softmax(output[1] / temperature, 1), output[2].view({x.sizes()[0], -1})};
      }
                                                      ...
                                                      torch::Tensor value_targets)
      {
        ...
        torch::Tensor value_loss = mse_loss(value_predictions, value_targets / 10);
      ...
      // torch::Tensor ResModelImpl::layer_j(torch::Tensor x, int j)
      // {
      //   for (int i = 0; i < num_torso_blocks_ + 2; i++)
      //   {
      //     if (i == 0)
